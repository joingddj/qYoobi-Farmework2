...
public class DefaultCoinProcessor implements CoinProcessor {
    ...
    public void generateKLine(int range, int field, long time) {
        ...
        if(field == Calendar.MINUTE || field == Calendar.HOUR_OF_DAY || field == Calendar.DAY_OF_YEAR){
            ...
            processKline(kLine, fromTime, endTime, field);
        }
    ...
    // 处理周K线和月K线的更具效率的方法
    public void processKline(KLine kline, String fromTime, String endTime, int field){
        // 查询过去时间段的日线（7条）
        List<KLine> lines = service.findAllKLine(symbol, Long.valueOf(fromTime), Long.valueOf(endTime),"1day");
        if(lines.size() > 0) {
