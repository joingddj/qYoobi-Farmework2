        ...
        try {
            ...
            if (StringUtils.isNotBlank(message)) {
                ...
                if (message.indexOf("ping") > 0) {
                    ...
                    String symbol = sb.toString().replace("-", "/").toString().toUpperCase();
                    ...
                    if (type.equals("kline")) {
                        ...
                        if (null != data && !"".equals(data) && JSONUtils.isJsonArray(data)) {
                            ...
                            for (int i = 0; i < klineList.size(); i++) {
                                ...
                                BigDecimal close = klineObj.getBigDecimal("close"); // 收盘价（当K线为最晚的一根时，是最新成交价）
                                BigDecimal high = klineObj.getBigDecimal("high"); // 最高价
                                BigDecimal low = klineObj.getBigDecimal("low"); // 最低价
                                BigDecimal amount = klineObj.getBigDecimal("amount"); // 成交量(币), 即 sum(每一笔成交量(张)*单张合约面值/该笔成交价)。 值是买卖双边之和
                                BigDecimal vol = klineObj.getBigDecimal("vol"); // 成交量张数。 值是买卖双边之和
                                BigDecimal tradeTurnover = klineObj.getBigDecimal("trade_turnover");//成交额, 即sum（每一笔成交张数 * 合约面值 * 成交价格）。 值是买卖双边之和
                                int count = klineObj.getIntValue("count"); // 成交笔数
                                long time = klineObj.getLongValue("id");
                                long mrid = klineObj.getLongValue("mrid");//订单id
                                ...
                                kline.setTime(time);
                                kline.setTurnover(tradeTurnover);
                                kline.setVolume(vol);
                        ...
                        if (StringUtils.isNotEmpty(tick) && JSONUtils.isJsonObject(tick)) {
                            ...
                            for (int i = 0; i < asks.size(); i++) {
                                TradePlateItem item = new TradePlateItem();
                                JSONArray asksItemObj = asks.getJSONArray(i);
                                if (asksItemObj.size() == 2) {
                                    item.setPrice(asksItemObj.getBigDecimal(0));
                                    item.setAmount(asksItemObj.getBigDecimal(1));
                                    sellItems.add(item);
                                }
            ...
            e.printStackTrace();
            logger.error("[WebSocketHuobi] websocket exception: {}", e);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[WebSocketHuobi] websocket exception: {}", e);
        }
