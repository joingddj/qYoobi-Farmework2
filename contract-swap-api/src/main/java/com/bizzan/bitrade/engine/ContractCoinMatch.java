                ...
                if(wallet.getUsdtPattern() == ContractOrderPattern.FIXED) { // 逐仓
                    ...
                    if(wallet.getUsdtSellPrice().compareTo(BigDecimal.ZERO) > 0) {
                        ...
                        if (sellPL.compareTo(BigDecimal.ZERO) < 0) {
                            ...
                            BigDecimal minNeedPrinc = wallet.getUsdtSellPosition().add(wallet.getUsdtFrozenSellPosition()).multiply(wallet.getUsdtShareNumber()).divide(wallet.getUsdtSellLeverage());
