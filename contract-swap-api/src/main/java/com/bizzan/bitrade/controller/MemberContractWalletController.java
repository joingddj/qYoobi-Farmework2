...
public class MemberContractWalletController {
    ...
    public MessageResult getWalletList(@SessionAttribute(SESSION_MEMBER) AuthMember authMember) {
        ...
        for (MemberContractWallet wallet : list) {
            ...
            ContractCoinMatch coinMatch = contractCoinMatchFactory.getContractCoinMatch(wallet.getContractCoin().getSymbol());
            BigDecimal currentPrice = BigDecimal.ZERO;
            if (coinMatch!=null){
                currentPrice = coinMatch.getNowPrice();
            }else{
                log.info("【警告】信息获取不完整，请排查数据[0001]："+JSONObject.toJSON(wallet));
            }
            // 计算金本位权益（多仓 + 空仓）
