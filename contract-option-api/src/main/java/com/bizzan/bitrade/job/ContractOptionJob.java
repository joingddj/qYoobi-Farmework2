            ...
            for (int i = 0; i < coinList.size(); i++) {
                ...
                for(int j = 0; j < options.size(); j++) {
                    ...
                    if(timeGap/1000 >= coinList.get(i).getOpenTimeGap() - 3) { // 3秒延迟
                        ...
                        temOption.setOpenPrice(factory.getContractCoinMatch(coinList.get(i).getSymbol()).getNowPrice());
                        optionService.save(temOption);
                        logger.info("{} - 第 {} 期预测合约状态变化：投注中 => 开奖中", temOption.getSymbol(), temOption.getOptionNo());
                        ...
                        if(temOption.getStatus() == ContractOptionStatus.STARTING
                            ...
                            // 状态是下注中 且 买涨奖池为空  且  预测合约交易对的设置了初始买涨奖池
                ...
                for(int j = 0; j < optionsOpening.size(); j++) {
                    ...
                    if(timeGap/1000 >= coinList.get(i).getCloseTimeGap() - 3) { // 3秒延迟
                        ...
                        temOption.setClosePrice(factory.getContractCoinMatch(coinList.get(i).getSymbol()).getNowPrice());

                        logger.info("{} - 第 {} 期预测合约状态变化：开奖中 => 已开奖", temOption.getSymbol(), temOption.getOptionNo());
                        ...
                        if(temOption.getClosePrice().subtract(temOption.getOpenPrice()).compareTo(BigDecimal.ZERO) > 0) {
                            logger.info("{} - 第 {} 期预测合约开奖结果：涨", temOption.getSymbol(), temOption.getOptionNo());
                            ...
                            logger.info("{} - 第 {} 期预测合约开奖结果：平", temOption.getSymbol(), temOption.getOptionNo());
                            ...
                            logger.info("{} - 第 {} 期预测合约开奖结果：跌", temOption.getSymbol(), temOption.getOptionNo());
